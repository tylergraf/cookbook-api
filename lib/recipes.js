...
exports.get = function(req, res, next) {
  ...
  Recipe.findOneAndUpdate({ _id: id }, { $inc: { views: 1 }}).populate({ path: '_subcategory', populate: { path: '_category' }})
    .then(recipe=>{
...
exports.popular = function(req, res, next) {
  ...
  Recipe.find()
    ...
    .then(recipe=>{
      if(!recipe.length) return res.status(404).end();
      console.log(recipe);
      debug('Recipe:',recipe);
...
exports.list = function(req, res, next) {
  ...
  Recipe.find({_subcategory: subcategoryId}).populate({ path: '_subcategory', populate: { path: '_category' }}).sort({ title: 1})
    .then(recipes=>{
