...
exports.get = function(req, res, next) {
  ...
  if(req.headers.count === 'false'){
    count = 0;
  }

  var calls = [Recipe.findOneAndUpdate({ _id: id }, { $inc: { views: count }}).populate({ path: '_subcategory', path: '_user', populate: { path: '_category' }}).exec()];
  if(userId){
...
exports.popular = function(req, res, next) {
  ...
  Promise.all([
    ...
    Recipe.find()
      .sort('-views')
      .limit(30)
      .populate({ path: '_subcategory', path: '_user', populate: { path: '_category' }})
  ])
...
exports.list = function(req, res, next) {
  ...
  Promise.all([
    ...
    Recipe.find({_subcategory: subcategoryId})
      ...
      .populate('_subcategory._category')
      .sort({ title: 1})
    ])
    .then(([favorites,recipes])=>{
      if(!recipes.length) return res.status(404).end();
...
exports.all = function(req, res, next) {
  // Promise.all([
    // Favorite.find(),
    Recipe.find()
      // .populate({ path: '_subcategory', path: '_user', })
      .populate({path:'_subcategory', populate: '_category'})
      .populate('_user')
      // .sort({ title: 1})
    // ])
    // .then(([favorites,recipes])=>{
    ...
    .then(recipes=>{
      ...
      res.json(recipes.map(r=>{
        r.ingredients = r.md_ingredients
        r.directions = r.md_directions
        return r
      }))
    })
    .catch(err=>next(err));
};
exports.userRecipes = function(req, res, next) {
  const userId = req.user && req.user.id;
  const uid = req.params.uid;

  User.findOne({uid: uid.toString()})
    .then(user=>{
      return Promise.all([
        Favorite.find({_user: userId}),
        Recipe.find({_user: user._id})
          .populate({ path: '_subcategory', path: '_user', populate: { path: '_category' }})
          .sort({ title: 1})
        ]);
    })
...
exports.search = function(req, res, next) {
  ...
  Recipe.find({$text: {$search: `${searchTerm}`}}, {score: {$meta: "textScore"}}).sort({score:{$meta:"textScore"}}).limit(50).exec(function (err, searchResults) {
