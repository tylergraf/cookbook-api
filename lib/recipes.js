...
exports.list = function(req, res, next) {
  ...
  Recipe.find({_subcategory: subcategoryId}).populate('_subcategory').sort({ title: 1}).exec(function (err, recipes) {
    if(err) return next(err);

    req.subcategory = (recipes.length) ? recipes[0]._subcategory : null;
